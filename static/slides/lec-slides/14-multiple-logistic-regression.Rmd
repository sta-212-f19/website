...
.small[
```{r}
glm(Acceptance ~ GPA + MCAT, data = MedGPA) %>%
  glance()
```
]
...
.small[
```{r}
pchisq(9.651883 - 9.378368, df = 1, lower.tail = FALSE)
```
]
...
.small[
```{r}
glm(Acceptance ~ GPA + MCAT + Apps, data = MedGPA) %>%
  glance()
```
]
...
.small[
```{r}
pchisq(9.651883 - 9.369395, df = 2, lower.tail = FALSE)
```
]
...
* We want to compare a model with GPA, MCAT, and Apps to one with only GPA
